...
export async function runDemoCycle({ claimStore, provenanceChain, embeddingProvider, anomalyDetector, fastify, tier: rawTier = 'standard' }) {
  const tierConfig = TIER_CONFIG[rawTier];
  if (!tierConfig) throw new ValidationError(`tier must be one of: ${Object.keys(TIER_CONFIG).join(', ')}`);

  const threadId = randomUUID();
  const cycleId = generateId('syn9_cycle');

  const cexResult = await getCexPrice();
  const onchainFeedClaim = await demoWeave({ claimStore, provenanceChain, embeddingProvider, anomalyDetector, threadId, payload: { asset: 'OKB', priceUsd: cexResult.price, source: cexResult.source, mechanism: 'cex_orderbook', timestamp: Date.now(), note: `OKB CEX spot price: $${cexResult.price} USDT (OKX orderbook). Demo run.` }, fastify });
  ...
  const signalClaim = await demoWeave({ claimStore, provenanceChain, embeddingProvider, anomalyDetector, threadId, payload: { asset: 'OKB', priceUsd: dexResult.price, source: dexResult.source, mechanism: 'dex_aggregator_onchain', chainIndex: '196', timestamp: Date.now(), cexPriceForComparison: cexResult.price, divergencePct: Number(divergencePct.toFixed(6)), contradictionDetected, note: `OKB DEX aggregator quote (XLayer): $${dexResult.price} USDT. Divergence: ${divergencePct.toFixed(4)}%. Demo run.` }, fastify });
  ...
  return {
    ...
    opportunity: {
      ...
      recommendation: contradictionDetected
        ...
        : 'MONITOR — sources agree; no immediate arb signal',
    },
    provenance: [
      { sourceId: 'onchain_feed', entryId: onchainFeedClaim.claimId, chainHash: onchainFeedClaim.chainHash, mechanism: 'cex_orderbook', ingestedAt: onchainFeedClaim.createdAt, confidence: 0.95, priceObserved: cexResult.price },
      { sourceId: 'signal_analyst', entryId: signalClaim.claimId, chainHash: signalClaim.chainHash, mechanism: 'dex_aggregator_onchain', ingestedAt: signalClaim.createdAt, confidence: 0.92, priceObserved: dexResult.price },
    ],
