...
function buildDemoResult({ cycleId, threadId, tier, cexResult, dexResult, divergencePct, contradictionDetected, confidence, onchainFeedClaim, signalClaim, conflicts }) {
  return {
    cycle_id: cycleId,
    thread_id: threadId,
    tier,
...
// Named export — shared by /v1/demo/research-cycle (the HTTP route) and
// /v1/core (which auto-seeds a demo thread when no threadId is supplied).
export async function runDemoCycle({ claimStore, provenanceChain, embeddingProvider, anomalyDetector, fastify, tier: rawTier = 'standard' }) {
  ...
  if (!tierConfig) {
    throw new ValidationError(`tier must be one of: ${Object.keys(TIER_CONFIG).join(', ')}`);
  }
  ...
  return buildDemoResult({ cycleId, threadId, tier: rawTier, cexResult, dexResult, divergencePct, contradictionDetected, confidence, onchainFeedClaim, signalClaim, conflicts });
}
